...
/* ── config ──────────────────────────────────────────── */
const LINES = [
  { scrambled: "EHT CAINERMA DNATSREDNU", answer: "THE AMERICAN UNDERSTANDS" },
  { scrambled: "HATW RONELAE DERIT OT LLET MEHT", answer: "WHAT ELEANOR TRIED TO TELL THEM" },
  { scrambled: "EHT MUSTBANE SDROCER EVORP GNIHTYREVE", answer: "THE BASEMENT RECORDS PROVE EVERYTHING" },
  { scrambled: "LWELDAC SKNHIT EHS NAC LORTNOC", answer: "CALDWELL THINKS SHE CAN CONTROL" },
  { scrambled: "EHT EVITARRAN TUB EHT HTURT SAH MUTNMEOM WON", answer: "THE NARRATIVE BUT THE TRUTH HAS MOMENTUM NOW" },
];
...
export default function M1C7() {
  ...
  const handleLineSubmit = async (index: number) => {
    ...
    if (normalized === expected) {
      ...
      // Lock in the canonical answer text
      const newInputs = [...inputs];
      newInputs[index] = LINES[index].answer;
      setInputs(newInputs);

      // Check if all 5 are now correct
        ...
        <div className="px-12 pt-14 pb-12 flex flex-col items-center text-center">
          ...
          {!showReward && !solved && (
            <p className="text-base mb-8" style={{ color: "rgba(230,230,230,0.45)" }}>
              Decode each intercepted fragment below.
            </p>
          )}
          ...
          {showReward && (
            <div className="w-full text-center" style={{ animation: "fade-in-up 0.8s ease-out both" }}>
              ...
              <h2 className="text-xl tracking-wider mb-6" style={{ fontFamily: font, color: textColor }}>
                EXPANDED INTERCEPT — FULL TRANSMISSION RECOVERED
              </h2>
